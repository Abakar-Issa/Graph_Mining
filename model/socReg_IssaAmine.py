...
from utility.similarity import pearson_sp, cosine_sp, jacc_sp,sorenzen_dice,minkowski_distance,Chebyshev_dist
...
class SocialReg(MF):
    ...
    def get_sim(self, u, k):
        ...
        # on utilise la sim jacc
        sim_jacc = (jacc_sp(self.rg.get_row(u), self.rg.get_row(k)) + 1.0) / 2.0
        # ou sorenzen_dice
        dice = (sorenzen_dice(self.rg.get_row(u), self.rg.get_row(k)) + 1.0) / 2.0
        # minkowski_distance
        minkowski = (minkowski_distance(self.rg.get_row(u), self.rg.get_row(k),1) + 1.0) / 2.0
        Chebyshev_distance = (Chebyshev_dist(self.rg.get_row(u), self.rg.get_row(k)) + 1.0) / 2.0
